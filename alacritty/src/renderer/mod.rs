...
impl TextShaderProgram {
    pub fn new() -> Result<TextShaderProgram, ShaderCreationError> {
        ...
        let (u_scale, u_atlas) = unsafe {
            (
                gl::GetUniformLocation(program, cptr!(b"u_scale\0")),
                gl::GetUniformLocation(program, cptr!(b"u_atlas\0")),
            )
        };

        assert_uniform_valid!(u_scale, u_atlas);

        let shader = Self { id: program, u_scale, u_atlas };
